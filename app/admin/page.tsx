  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 text-slate-50 font-sans">
        <div className="bg-slate-900 border border-emerald-900/50 p-10 rounded-2xl shadow-2xl shadow-emerald-900/20 max-w-md w-full text-center">
          <div className="w-16 h-16 bg-emerald-600/20 border border-emerald-500 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6 text-2xl">🛡️</div>
          <h1 className="text-3xl font-bold mb-2 uppercase tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">Iron Keep HQ</h1>
          <p className="text-emerald-400 text-sm mb-8 tracking-widest uppercase">Admin Matrix Restricted</p>
          <form onSubmit={handleLogin} className="space-y-6">
            <input type="password" value={passcode} onChange={(e) => setPasscode(e.target.value)} placeholder="ENTER PASSCODE" className="w-full bg-slate-950 border border-slate-700 rounded-lg p-4 text-center tracking-[0.5em] text-white focus:outline-none focus:border-emerald-500 transition-colors" />
            <button type="submit" className="w-full bg-gradient-to-r from-emerald-500 to-cyan-500 text-slate-950 font-bold py-4 rounded-lg transition-all uppercase tracking-widest text-sm shadow-lg shadow-emerald-900/50 hover:opacity-90">Initialize Engine</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 font-sans relative">
      <div className="max-w-6xl mx-auto flex justify-between items-center mb-8 border-b border-slate-800 pb-5">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">IRON KEEP CRM</h1>
          <p className="text-sm text-slate-400 mt-1">Owner Database Matrix Engine</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => setIsAuthenticated(false)} className="flex items-center gap-2 bg-rose-500/10 border border-rose-500/20 px-4 py-2 rounded-lg text-sm font-semibold text-rose-400 hover:bg-rose-500/20 transition-all">Lock System</button>
          <button onClick={fetchDashboardData} className="flex items-center gap-2 bg-slate-900 border border-slate-800 px-4 py-2 rounded-lg text-sm hover:bg-slate-800 text-slate-300 transition-all"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} /> Sync</button>
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-4 mb-6">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search athletes by name or phone..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-900/50 border border-slate-800 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-emerald-500/50 transition-all" />
        </div>
        <div className="flex gap-2">
          {['all', 'active', 'expired'].map(status => (
            <button key={status} onClick={() => setStatusFilter(status)} className={`px-4 py-2 rounded-xl text-sm font-semibold capitalize transition-all border ${statusFilter === status ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-slate-900/50 border-slate-800 text-slate-400 hover:bg-slate-800'}`}>{status}</button>
          ))}
        </div>
      </div>

      <div className="max-w-6xl mx-auto bg-slate-900/30 border border-slate-800/80 rounded-2xl p-6">
        {loading ? <div className="text-center text-slate-500 py-8">Querying schema...</div> : filteredMembers.length === 0 ? <div className="text-center text-slate-500 py-12">No records match your filters.</div> : (
            ...
            {filteredMembers.map((member) => (
              <div key={member.id} className="bg-slate-900/60 border border-slate-800/60 p-4 rounded-xl flex justify-between items-center group">
                ...
                <div className="flex items-center gap-4">
                  <button onClick={() => handleCheckIn(member.id, member.full_name)} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 border border-slate-700 rounded-lg text-xs font-bold text-slate-300 hover:bg-emerald-500/20 hover:text-emerald-400 hover:border-emerald-500/30 transition-all"><MapPin className="w-3 h-3" /> Check In</button>
